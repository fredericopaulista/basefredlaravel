          ...
          <tr>
            <td>
              <div class="bg-primary text-white w-12 h-12 rounded-full inline-flex items-center align-middle justify-center font-bold text-4xl mx-4">
                <img width="70%" height="70%" src="{{ $category->getFirstMediaUrl('categories', 'thumb') }}">
              </div>
            </td>
            <td>
                <a href="{{ route('categorias.edit', $category->id) }}" class="font-bold hover:text-primary text-sm">  {{ $category->name }}</a>
            ...
            <td><a class="btn btn-success"
                href="{{ route('categorias.edit', $category->id) }}"><i class="material-icons mx-2" > edit</i>Editar</a>

        </td>
        <td> <form method="post" action="{{ route('categorias.destroy', $category->id) }}">
            @csrf
            @method('delete')
            <button type="submit"
