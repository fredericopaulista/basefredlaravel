        ...
        <div class="container">
          ...
          <div class="d-flex justify-content-between align-items-center">
            ...
            <ol>
              ...
              <li><a href="{{ route('site.tags', ['tag' => $tag->slug . config('APP_CIDADE_SLUG'), 'tagid' => $tag->id, 'service' => $servicePhoto->id]) }}" title="{{ $tag->name }} {{ $citydata->name }}">{{ $tag->name }} {{ $citydata->name }}</a></li>
                    ...
                    @if (preg_match(
                        ...
                        <a id="whats-menu"
                            href="https://api.whatsapp.com/send?phone={{ $configuration->whatsapp }}&amp;text=Entrei em contato através do site {{ Request::url() }}"
                             title="Orçamento" target="_blank"><img src="{{ asset('storage/' . $configuration->banner_gif)}}" alt="{{ $servicePhoto->name }}" width="99%"></a>
                    @else
                        <a id="whats-menu"
              ...
              <div class="sidebar-item recent-posts">
                ...
                <div class="mt-3">
                  ...
                  <div class="post-item mt-3">
                    <img src="{{ $related_service->getFirstMediaUrl('services') }}" alt="{{ $related_service->title }}" class="flex-shrink-0">
                    <div>
                      <h4><a href="{{ route('site.servico', ['category' => $related_service->category->slug . $cityslug, 'service' => $related_service->slug . $cityslug]) }}">{{ $related_service->title }}</a></h4>
