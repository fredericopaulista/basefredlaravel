<div class="alert text-center cookiealert" role="alert">
    <b>Você gosta de cookies?</b> &#x1F36A; Nosso site utiliza cookies para melhorar a navegação. Ao utilizar nosso
    site, você concorda com nossas políticas. <a href="{{ route('site.policy') }}" title="Políticas de Privacidade"
            ...
            <div class="row">
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="footer-info">
                        <a href="{{ env('APP_URL') }}" title="{{ $configuration->company }}"
                            alt="{{ $configuration->company }}">
                        ...
                        <p>
                            ...
                            <strong>Telefone:</strong> <a href="tel:{{ $configuration->phone }}"
                                title="Telefone Contato">{{ $configuration->phone }}</a><br>
                            ...
                            <strong>Email:</strong> <a
                                href="mailto:{{ $configuration->email }}">{{ $configuration->email }}</a><br>
                        </p>
                    </div>
                </div>
                ...
                <div class="col-lg-2 col-md-6 footer-links">
                    <h4>Links Úteis</h4>
                    <ul>
                        <li><i class="bi bi-chevron-right"></i> <a href="{{ route('site.mapadosite') }}"
                                title="Mapa do Site">Mapa do Site</a></li>
                        <li><i class="bi bi-chevron-right"></i> <a href="{{ env('APP_URL') }}"
                                title="{{ $configuration->company }}">{{ $configuration->company }}</a></li>
                        <li><i class="bi bi-chevron-right"></i> <a href="{{ route('site.contato') }}"
                                title="Contato">Contato</a></li>

                        @foreach ($pages as $page)
                            <li><i class="bi bi-chevron-right"></i> <a href="{{ env('APP_URL') }}/{{ $page->slug }}"
                                    alt="{{ $page->title }}" title="{{ $page->title }}">{{ $page->title }}</a>
                            </li>
                        @endforeach
                ...
                <div class="col-lg-3 col-md-6 footer-links">
                    ...
                    <ul>
                        @foreach ($navlinks as $category)
                            <li><a class="bi bi-chevron-right "
                                    href="{{ route('site.single', $category->slug . config('APP_CIDADE_SLUG')) }}"
                                    title="{{ $category->name . ' ' . config('APP_CIDADE') }}">{{ $category->name }}</a>
                            </li>
                        @endforeach
                        ...
                        <div class="form-group col-md-6">
                            <div class="input-group mt-6">
                                ...
                                <div class="input-group-addon" style="margin-top:20px">
                                    <img src="{{ asset('site/assets/img/soma.jpg') }}"
                                        style="margin-right:10px;height:30px;">
                                </div>
                                <input type="hidden" name="n1" value="<?php echo $n1; ?>" />
                                <input type="hidden" name="n2" value="<?php echo $n2; ?>" />
                                <input type="hidden" name="result" value="<?php echo $result; ?>" />
                                <input type="text" name="soma_user" class="form-control" id="resposta"
                                    placeholder="Resposta" required="required" " style="margin-right:10px;height:30px;margin-top: 20px;"/>
                    </div>
